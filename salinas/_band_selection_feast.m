function band_selection_feast( ratio, pos )

  load all_classes_nrmlz.txt
  ...
  for i = 1 : rows(positive_mat)
	...
	test_data = [ test_data; positive_mat(i, :) ];
  ...
  for i = 1 : rows(negative_mat)
	...
	test_data = [ test_data; negative_mat(i, :) ];
	testing_labels = [ testing_labels; -1 ];
      end
  end

  save "feast_data_salinas.txt" training_data test_data testing_labels training_labels
