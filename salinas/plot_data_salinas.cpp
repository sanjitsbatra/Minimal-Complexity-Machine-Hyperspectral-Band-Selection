...
int get_ratio_index( const float r )
{
    ...
    else if (r > 0.95 - 1e-3 && r <= 0.95 + 1e-3) { return 5;  std::cerr << "index is 4\n";}  // 0.96
    { std::cerr << "die\n"; exit(0); }
}
...
int main( int argc, char *argv[] )
{
    ...
    const float R =  ::atof( argv[2] );  // ratio
    const int NB =   ::atoi( argv[3] );  // num bands
    const float GM = ::atof( argv[4] );  // gamma

    std::cerr << "ratio " << R << " Num_band " << NB << " gamma " << GM << "\n";

    float mcc_table[NBANDS][NCLASSES];  // mcc table
    float ber_table[NBANDS][NCLASSES];  // ber table
    float dp_table[NBANDS][NCLASSES];   // data points table

    float mcc_table_r[NRATIO][NCLASSES];  // mcc table for ratio
    float ber_table_r[NRATIO][NCLASSES];  // ber table for ratio
    float dp_table_r[NRATIO][NCLASSES];   // data points table for ratio
